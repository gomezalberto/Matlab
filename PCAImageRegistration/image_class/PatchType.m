...
% Class to handle patches. Inherits from the ImageType class
%
% Author: Alberto Gomez, Biomedical Engineering, KCL, 2011
    ...
    methods(Access = public)
        ...
        function patch = extractPatch(obj, index, s, border)
            ...
            patch = PatchType(s,obj.origin,obj.spacing,obj.orientation);
            if numel(obj.spacing)==1
                patch.data = zeros((s+preborder+postborder),1);
            else
        ...
        function onedindex = nd_to_oned_index(obj, nd_index_patch)
            ...
            if numel(obj.spacing)==1
                ...
                for ii=1:ndims(obj.data)
                    str = [str 'index_local(' num2str(ii) ',:)'','];
                end
