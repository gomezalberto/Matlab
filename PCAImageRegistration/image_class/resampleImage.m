...
% resampleImage Fast resample an image using a reference image
% out = resampleImage(im,ref)
% out = resampleImage(im,ref,'interpolation',interp)
% out = resampleImage(im,0,'spacing',spacing)
% out = resampleImage(im,ref,’blur’,[Nx,Ny,Nz,Nt],[sigma_x, sigma_y, sigma_z, sigma_t])
%
...
% Set interpolation
    if (strcmp(varargin{i},'interpolation'))
        interp=varargin{i+1};
% Set spacing of the resampled image
...
% Set size of maximum cube used for resampling
    elseif (strcmp(varargin{i},'maxChunkSize'))
        MAX_CHUNK_SIZE = varargin{i+1};
        i=i+1;
% Enlarge borders maintains the FoV of the warped image
...
if (ndims_==4)
    ...
    for i=1:size(intervals,1)
        ...
        clear x y z t;
        datas = im_.GetValue(positions,interp); % Problem is here
